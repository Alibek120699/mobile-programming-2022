...
export const visits = declareAtom({ status: 'notasked' }, on => [
  ...
  on(actions.getVisitsFail, (_, payload) => ({ status: 'failed', error: payload })),
...
export const visitsCounter = declareAtom({ status: 'notasked', data: initialVisitsCounter }, on => [
...
export const visitsPaginationLimit = declareAtom({ status: 'notasked', data: 1 }, on => [
  on(actions.getVisitsSuccess, (_, payload) => ({
    status: 'success',
    data: payload.pages || 1,
  })),
]);
